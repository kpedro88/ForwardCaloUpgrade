...
#define maxHDe 7 //energy points for hadrons
...
class Sample {
	public:
		Sample() : dir(""), fpre_hcal_elec(""), fpre_ecal_elec(""), fpre_pion(""), ecal_name(""), ecal_name_rat(""), hcal_name(""), hcal_name_rat(""), 
				   sam_ecal(1), sam_hcal_elec(1), sam_hcal_pion(1), sam_init(false), zeroWt(0.5), zcalWt(0.0), supp_info(""), 
				   eh_hcal(-1), eh_err_hcal(-1), k_hcal(-1), k_err_hcal(-1), eh_ecal(-1), eh_err_ecal(-1), k_ecal(-1), k_err_ecal(-1), response_hcal(0), response_ecal(0) {}
		std::string dir;
		std::string fpre_hcal_elec, fpre_ecal_elec, fpre_pion;
		std::string ecal_name, ecal_name_rat, hcal_name, hcal_name_rat;
		Double_t sam_ecal;
		Double_t sam_hcal_elec;
		Double_t sam_hcal_pion;
		bool sam_init;
		Double_t zeroWt;
		Double_t zcalWt;
		std::string supp_info;
		Double_t eh_hcal, eh_err_hcal, k_hcal, k_err_hcal;
		Double_t eh_ecal, eh_err_ecal, k_ecal, k_err_ecal;
		TGraphErrors* response_hcal, response_ecal;
		TF1* ecal_cut;
};

std::map<int,Sample*> sample_map;

void set_ecal_cut(int snum, TF1* f) { Sample* sp = sample_map[snum]; sp->ecal_cut = f; }

void add_sample(int snum, std::string dir_, std::string fpre_hcal_elec_, std::string fpre_ecal_elec_, std::string fpre_pion_, std::string ecal_name_, std::string ecal_name_rat_, 
				std::string hcal_name_, std::string hcal_name_rat_, Double_t zeroWt_, Double_t zcalWt_, std::string supp_info_=""){
	Sample* sp = new Sample();
	sp->dir = dir_;
	sp->fpre_hcal_elec = fpre_hcal_elec_;
	sp->fpre_ecal_elec = fpre_ecal_elec_;
	sp->fpre_pion = fpre_pion_;
	sp->ecal_name = ecal_name_;
	sp->ecal_name_rat = ecal_name_rat_;
	sp->hcal_name = hcal_name_;
	sp->hcal_name_rat = hcal_name_rat_;
	sp->zeroWt = zeroWt_;
	sp->zcalWt = zcalWt_;
...
void init_samples(int v0=0){
	...
	TF1* nocut = new TF1("f1","0",energies[0],energies[maxHDe-1]);
	//default ecal+hcal
	...
	add_sample(0,"/data/users/pedrok/FullSim/wlyso","hcal_only_elec","hcal_pbwo4_elec","hcal_pbwo4_pion","PbWO_{4} ECAL","PbWO_{4}","HCAL","HCAL",w0[v0],0.0);
	TF1* f0 = new TF1("f0","0.19*x+1.8",energies[0],energies[maxHDe-1]);
	//TF1* f0 = new TF1("f0","0",energies[0],energies[maxHDe-1]);
	set_ecal_cut(0,f0);
	//wlyso ecal w/out preshower & dead material, + new dead material: 6 cm Al
	double w1[] = {0.5,0.40,0.33};
	add_sample(1,"/data/users/pedrok/FullSim/wlyso","hcal_only_elec","wlyso5_elec","wlyso5_pion","W-LYSO ECAL","W-LYSO","HCAL","HCAL",w1[v0],0.0,"(6 cm Al dead mat.)");
	//TF1* f1 = new TF1("f1","0.17*x-1.5",energies[0],energies[maxHDe-1]);
	//TF1* f1 = new TF1("f1","0.13*x+0.27",energies[0],energies[maxHDe-1]);
	TF1* f1 = new TF1("f1","0.20*x-2.77",energies[0],energies[maxHDe-1]);
	//TF1* f1 = new TF1("f1","0",energies[0],energies[maxHDe-1]);
	set_ecal_cut(1,f1);
	//wlyso ecal w/out preshower & dead material, + new dead material: 6 cm Al, Tungsten HE 18 layers (physics list QGSP_BERT_EMV)
	double w2[] = {0.5,0.37,0.37};
	add_sample(2,"/data/users/pedrok/FullSim/wlyso/qgsp_bert_emv","whcal_only_elec","wlyso_whcal_elec","wlyso_whcal_pion","W-LYSO ECAL","W-LYSO","WHCAL","WHCAL",w2[v0],0.0,"(WHCAL, Q_B_E)");
	set_ecal_cut(2,f1);
	//wlyso ecal w/out preshower & dead material, + new dead material: 6 cm Al, Tungsten HE 12 layers (physics list QGSP_BERT_HP)
	double w3[] = {0.5,0.35,0.35};
	add_sample(3,"/data/users/pedrok/FullSim/wlyso/qgsp_bert_hp","whcal_only_elec","wlyso_whcal_elec","wlyso_whcal_pion","W-LYSO ECAL","W-LYSO","WHCAL","WHCAL",w3[v0],0.0,"(WHCAL, Q_B_H)");
	set_ecal_cut(3,f1);
}
...
std::pair<Double_t,Double_t> g4_sample(int snum, Double_t energy, bool do_pion, bool do_hcal, bool do_show, bool do_print=false, bool set_val=true){
	Sample* sp = sample_map[snum];
	if(!sp) { std::cout << "Sample " << snum << " is not loaded." << std::endl; return std::pair<Double_t,Double_t>(0.,0.); }

	//select correct file
	std::string fpre;
	if(do_pion) fpre = sp->fpre_pion;
	else if(do_hcal) fpre = sp->fpre_hcal_elec;
	else fpre = sp->fpre_ecal_elec;
	
	//make filenames
	std::stringstream drawname, cutname, fname, piname;
	...
	if(do_hcal){
		...
		if(do_pion) cutname << "(" << sp->sam_ecal << "*ecal)<1000";
		else cutname << "";
	}
	else{
		drawname << "(ecal)/1000>>hsam(200)";
		cutname << "";
	}
	...
	if (do_show){
		...
		//formatting
		hsam->SetTitle("");
		if(do_hcal) hsam->GetXaxis()->SetTitle("E_{hcal} [GeV]");
		else hsam->GetXaxis()->SetTitle("E_{ecal} [GeV]");
		//hsam->SetStats(kTRUE);
		...
		if(do_print) {
			...
			oname << pdir << "/";
			if(do_pion && do_hcal) oname << "hcal_pion";
			else if(!do_pion && do_hcal) oname << "hcal_elec";
			else if(!do_pion && !do_hcal) oname << "ecal_elec";
			oname << "_sample_" << energy << "gev_peak.png";
	...
	if(set_val){
		if(do_pion && do_hcal) sp->sam_hcal_pion = f;
		else if(!do_pion && do_hcal) sp->sam_hcal_elec = f;
		else if(!do_pion && !do_hcal) sp->sam_ecal = f;
	}
...
void get_sampling_factors(int snum, bool do_show = false){
	...
	if(!sp->sam_init){
		...
		//ecal sampling factor from elec
		g4_sample(snum,energy,0,0,0);
		//hcal sampling factor from elec
		g4_sample(snum,energy,0,1,0);
		//hcal sampling factor from pion
		g4_sample(snum,energy,1,1,0);
...
energyRes* get_res(int snum, Double_t energy, bool do_pion, bool do_hcal, bool use_f_pion, bool do_fit, bool do_show, bool do_print=false, bool do_batch=false, bool do_mip=true){
	Sample* sp = sample_map[snum];
	...
	if(do_hcal){
		if(use_f_pion) drawname << sp->sam_hcal_pion;
		else drawname << sp->sam_hcal_elec;
		drawname << "*(hcal+";
		if(sp->zcalWt>0) drawname << sp->zcalWt << "*zcal+";
		drawname << sp->zeroWt << "*zero)/1000";
		if(do_pion && do_mip) cutname << "(" << sp->sam_ecal << "*ecal)<1000";
		//else if(do_pion && !do_mip) cutname << "(" << sp->sam_ecal << "*ecal)>1000";
		else if(do_pion && !do_mip) cutname << "(" << sp->sam_ecal << "*ecal)/1000>" << sp->ecal_cut->Eval(energy);
		else cutname << "";
	}
	else{
		drawname << "(" << sp->sam_ecal << "*ecal)/1000";
		if(do_pion) cutname << "(" << sp->sam_ecal << "*ecal)/1000>" << sp->ecal_cut->Eval(energy); //energy cut for pions in ECAL
		else cutname << "";
	}
	...
	if (do_show){
		...
		if(do_hcal) pave->AddText(sp->hcal_name.c_str());
		else pave->AddText(sp->ecal_name.c_str());
		pave->AddText((piname.str()).c_str());
		pave->SetFillColor(0);
		pave->SetBorderSize(0);
		pave->SetTextFont(42);
...
TGraphErrors* g4_plot_res(int snum, int qty, bool do_pion, bool do_hcal, bool use_f_pion, bool do_fit, bool do_show, bool do_print=false, bool do_mip=true){
	Sample* sp = sample_map[snum];
	...
	for (int i = 0; i < maxHDe; i++){
		...
		if(qty==2){
			...
			std::pair<Double_t,Double_t> f_temp = g4_sample(snum,energy,do_pion,do_hcal,0,0,0);
			v = f_temp.first;
			ve = f_temp.second;
		}
		else {
			res_temp = get_res(snum,energy,do_pion,do_hcal,use_f_pion,do_fit,0,0,1,do_mip);
			...
			if(qty==1){
				...
				if(do_hcal) v = m/energy;
				else {
					//get hcal info for subtraction
					TF1* epi_hcal = new TF1("epi_hcal","(1+([0]-1)*([1]*log(x)))/[0]",energies[0],energies[maxHDe-1]);
					epi_hcal->SetParameter(0,sp->eh_hcal);
					epi_hcal->SetParameter(1,sp->k_hcal);
					v = m/(energy - epi_hcal->Eval(energy)*sp->response_hcal->GetY()[i]*energy);
					std::cout << "energy = " << energy << ", epi_hcal = " << epi_hcal->Eval(energy) << ", R_hcal = " << sp->response_hcal->GetY()[i]*energy << ", R_ecal = " << m << " -> v = " << v << std::endl;
				}
				//if(v>1) v=1;
				ve = me/energy;
			}
	...
	if(do_pion && do_hcal) qtyaxes[0] = sp->hcal_name_rat + " #pi^{-} Response (R_{hcal}/E_{gen})";
	else if(do_pion && !do_hcal) qtyaxes[0] = sp->ecal_name_rat + " #pi^{-} Response (R_{ecal}/E_{gen})";
	...
	if(qty==0 && do_pion){
		...
		if(do_hcal && sp->eh_hcal==-1){
			sp->eh_hcal = c;
			sp->eh_err_hcal = ce;
			sp->k_hcal = k;
			sp->k_err_hcal = ke;
			//sp->response_hcal = val_graph;
		}
		else if(!do_hcal && sp->eh_ecal==-1){
			sp->eh_ecal = c;
			sp->eh_err_ecal = ce;
			sp->k_ecal = k;
			sp->k_err_ecal = ke;
			//sp->response_ecal = val_graph;
		}
	...
	if(do_show){
		...
		std::string pavename;
		if(do_hcal) pavename = sp->hcal_name;
		else pavename = sp->ecal_name;
...
void calc_eh_ecal(int snum, bool do_fit=true, bool do_print=false){
	Sample* sp = sample_map[snum];
	if(!sp) { std::cout << "Sample " << snum << " is not loaded." << std::endl; return; }

	//get eh for hcal
	g4_plot_res(snum, 0, 1, 1, 0, do_fit, 1, do_print, 1);
	//get resp for hcal
	sp->response_hcal = g4_plot_res(snum, 0, 1, 1, 0, 1, 0, 0, 0);
	
	//get eh for ecal using above
	g4_plot_res(snum, 0, 1, 0, 0, do_fit, 1, do_print, 1);
}