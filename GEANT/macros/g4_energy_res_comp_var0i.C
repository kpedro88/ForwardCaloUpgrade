...
class Sample {
	public:
		Sample() : dir(""), fpre(""), gname(""), gname_rat(""), hom_ecal(true), sam_ecal(1), sam(1), sam_init(false), 
		           zeroWt(0.5), zcalWt(0), supp_info(""), sampling_term(0.0), sampling_term_err(0.0), constant_term(0.0), constant_term_err(0.0) {}
		std::string dir;
		std::string fpre;
		std::string gname;
...
void init_samples(){
	...
	add_sample(0,"/data/users/pedrok/FullSim/pbwo4","hcal","PbWO_{4} ECAL","PbWO_{4}",true,-1,0);
	//wlyso ecal w/ preshower & dead material
	add_sample(1,"/data/users/pedrok/FullSim/wlyso","wlyso","W-LYSO ECAL","W-LYSO",false,-1,0);
	//wlyso ecal w/out preshower
	//add_sample(2,"/data/users/pedrok/FullSim/wlyso","wlyso2","W-LYSO ECAL v2","W-LYSO v2",false);
	add_sample(2,"/data/users/pedrok/FullSim/wlyso","wlyso2","W-LYSO ECAL","W-LYSO",false,-1,0);
	...
	add_sample(3,"/data/users/pedrok/FullSim/wlyso","wlyso3","W-LYSO ECAL","W-LYSO",false,-1,0,"(no dead material)");
	//wlyso ecal, 33 layers, w/out preshower
	add_sample(4,"/data/users/pedrok/FullSim/wlyso","wlyso4","W-LYSO ECAL","W-LYSO",false,-1,0,"(33 layers)");
	//wlyso ecal w/out preshower & dead material, + new dead material: 6 cm Al
	add_sample(5,"/data/users/pedrok/FullSim/wlyso","wlyso5","W-LYSO ECAL","W-LYSO",false,-1,0,"(6 cm Al dead mat.)");
	//wlyso ecal w/out preshower & dead material, + new dead material: 6 cm Al, HE shifted forward
	add_sample(6,"/data/users/pedrok/FullSim/wlyso","wlyso6","W-LYSO ECAL","W-LYSO",false,-1,0,"(6 cm Al, HE fwd)");
	//wlyso ecal w/out preshower & dead material, + new dead material: 6 cm Al, HE shifted forward + 3 extra layers
	add_sample(7,"/data/users/pedrok/FullSim/wlyso","wlyso7","W-LYSO ECAL","W-LYSO",false,-1,0,"(Al, HE fwd + 3 lay)");
	//wlyso ecal w/out preshower & dead material, + new dead material: 6 cm Al, HE - 3 layers
	add_sample(8,"/data/users/pedrok/FullSim/wlyso","wlyso_zcal0_hcal14","W-LYSO ECAL","W-LYSO",false,-1,0,"(Al, HE -3 lay)");	
	//wlyso ecal w/out preshower & dead material, + new dead material: 6 cm Al, HE extension w/ 4 layers of 4.9mm W & 3.7 mm scint
	add_sample(71,"/data/users/pedrok/FullSim/wlyso","wlyso_z3cal","W-LYSO ECAL","W-LYSO",false,-1,0,"(Al, ext. HE 2#lambda_{0}(W))");
	...
	for(int i = 1; i <= 10; i++){
		...
		add_sample(i+30,"/data/users/pedrok/FullSim/wlyso",sn.str(),"W-LYSO ECAL","W-LYSO",false,-1,-1,ss.str());
	}
	...
	for(int i = 1; i <= 10; i++){
		...
		add_sample(i+40,"/data/users/pedrok/FullSim/wlyso",sn.str(),"W-LYSO ECAL","W-LYSO",false,-1,-1,ss.str());
	}	
//	for(int i = 1; i <= 5; i++){
//		stringstream sn;
//		sn << "wlyso_z2cal" << i;
...
//	}
	...
	add_sample(9,"/data/users/pedrok/FullSim/wlyso/qgsp_bert_emv","wlyso_whcal","W-LYSO ECAL","W-LYSO",false,-1,0,"(WHCAL, Q_B_E)");
	//wlyso ecal w/out preshower & dead material, + new dead material: 6 cm Al, Tungsten HE 18 layers (physics list QGSP_BERT_HP)
	add_sample(10,"/data/users/pedrok/FullSim/wlyso/qgsp_bert_hp","wlyso_whcal","W-LYSO ECAL","W-LYSO",false,-1,0,"(WHCAL, Q_B_H)");
}
...
//-------------------------------------------------------------
//forward declaration
std::pair<Double_t,Double_t> optimize_zeroWt(int snum, int num, Double_t sam_ecal, bool do_zcal);

//--------------------------------------
//function to calculate sampling factors
Double_t g4_sample(int snum, int num, Double_t sam_ecal, bool do_show, bool do_print=false, bool var_range=false, bool test_zero=false, Double_t test_wt=0.0, Double_t test_wt_zcal=0.0){
	if (num>=maxHDe || num<0) {
	...
	double zcalWt = sp->zcalWt;
	if(zeroWt == -1 && !test_zero) {
		//std::cout << "optimizing zero weight" << std::endl;
		std::pair<Double_t,Double_t> wts = optimize_zeroWt(snum,num,sam_ecal,sp->zcalWt == -1); //start recursion
		zeroWt = wts.first;
		if(zcalWt != 0) zcalWt = wts.second;
		sp->zeroWt = wts.first; //store value
		if(sp->zcalWt == -1) sp->zcalWt = wts.second; //store value
	}
	else if(test_zero) {
		zeroWt = test_wt; //use test value for weight
		if(zcalWt == -1) zcalWt = test_wt_zcal;
	}
	...
	if(var_range) {
		drawname << "(hcal+";
		if(zcalWt>0) drawname << zcalWt << "*zcal+";
		drawname << zeroWt << "*zero)/1000>>hsam(200)";
	}
	else {
		drawname << "(hcal+";
		if(zcalWt>0) drawname << zcalWt << "*zcal+";
		drawname << zeroWt << "*zero)/1000>>hsam(200,0," << energies[num]/100 << ")";
	}
	cutname << "(" << sam_ecal << "*ecal)<1000";
	totalTree->Draw((drawname.str()).c_str(),(cutname.str()).c_str(),"hist goff");
	TH1F* hsam = (TH1F*)gDirectory->Get("hsam");
	
	//when recursing, return RMS of histo
	//and close file to prevent too many open
	if(test_zero) {
		Double_t rms = hsam->GetRMS();
		_file->Close();
		return rms;
	}
...
std::pair<Double_t,Double_t> optimize_zeroWt(int snum, int num, Double_t sam_ecal, bool do_zcal){
	...
	//Double_t zeroStep = 0.001;
	int nsteps = (int)((zeroMax - zeroMin)/zeroStep);
	Double_t zeroWt = zeroMin;
	
	Double_t zcalMin = 0.0;
	Double_t zcalMax = 3.0;
	Double_t zcalStep = 0.01;
	//Double_t zcalStep = 0.001;
	int nsteps_zcal = (int)((zcalMax - zcalMin)/zcalStep);
	Double_t zcalWt = zcalMin;
	
	Double_t rmsMin = 1e10;
	Double_t zeroWtOpt = 0.0;
	Double_t zcalWtOpt = 0.0;
	
	//status values:
	//-1: not run, 0: 1D, 1: converged, 2: hit maxIt
	int status = -1;
	...
	Double_t zeroWtOptPrev = 0.0;
	Double_t zcalWtOptPrev = 0.0;
	
	if(do_zcal){
		//initial values
		zeroWtOpt = 0.5;
		while( (abs(zeroWtOpt-zeroWtOptPrev)>tol || abs(zcalWtOpt-zcalWtOptPrev)>tol) && it < maxIt){
			//save previous optima
			zeroWtOptPrev = zeroWtOpt;
			zcalWtOptPrev = zcalWtOpt;
			...
			//optimize zcalWt for given zeroWtOpt
			for(int j = 0; j < nsteps_zcal; j++){
				Double_t rmsTemp = g4_sample(snum,num,sam_ecal,0,0,1,1,zeroWtOpt,zcalWt);
				if(rmsTemp < rmsMin){
					rmsMin = rmsTemp;
					zcalWtOpt = zcalWt;
				}
			...
			//optimize zeroWt with new zcalWtOpt
			for(int j = 0; j < nsteps; j++){
				Double_t rmsTemp = g4_sample(snum,num,sam_ecal,0,0,1,1,zeroWt,zcalWtOpt);
				if(rmsTemp < rmsMin){
					rmsMin = rmsTemp;
					zeroWtOpt = zeroWt;
				}
		...
		for(int i = 0; i <= nsteps; i++){
			Double_t rmsTemp = g4_sample(snum,num,sam_ecal,0,0,1,1,zeroWt);
			//std::cout << "zeroWt = " << zeroWt << ", RMS = " << rmsTemp << std::endl;
			if(rmsTemp < rmsMin){
				rmsMin = rmsTemp;
				zeroWtOpt = zeroWt;
			}
	...
	std::cout << "Sample " << snum << ": optimal zero weight = " << zeroWtOpt;
	if(do_zcal) std::cout << ", optimal zcal weight = " << zcalWtOpt;
	if(status==1) std::cout << " (converged, deltas = " << abs(zeroWtOpt-zeroWtOptPrev) << ", " << abs(zcalWtOpt-zcalWtOptPrev) << ")";
	else if(status==2) std::cout << " (stopped, maxIt = " << maxIt << ")";
	std::cout << std::endl;
	
	return std::pair<Double_t,Double_t>(zeroWtOpt,zcalWtOpt);
}
...
TGraphErrors* g4_plot_res(int snum, int imip, int qty, bool do_fit, bool do_jet, bool do_show, bool do_print=false){
	...
	if(do_jet && qty==1){
		...
		std::cout << sp->gname << ((!sp->supp_info.empty()) ? " " + sp->supp_info : "") << ": " << sqrt(a)*100 << "%/sqrt(E) (+) " << sqrt(b)*100 << "%" << std::endl;
		//store in sample
