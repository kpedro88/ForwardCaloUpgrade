...
DetectorConstruction::DetectorConstruction():defaultMaterial(0),
 ...
 solidZcal(0),logicZcal(0),physiZcal(0),
 solZcalLayer(0),logZcalLayer(0),phyZcalLayer(0),
 solZcalAbs(0),logZcalAbs(0),phyZcalAbs(0),
 solZcalSens(0),logZcalSens(0),phyZcalSens(0), 
 solidZcalWrap(0),logicZcalWrap(0),physiZcalWrap(0),
 solidZcalGap(0),logicZcalGap(0),physiZcalGap(0),
...
// parameters for extra "zero calorimeter" in front of HE
//-------------------------------------------------------
  ZcalAbsThickness = 0.0*mm;
  ZcalSensThickness = 0.0*mm;
  NbOfZcalLayers = 0;
  ComputeZcalParameters();
  
// dead material in front of HE
//-----------------------------
  CablesThickness   = 23.40*cm;
  NewDeadThickness  = 0.0*cm; //disabled by default
  G10plateThickness = 2.5*mm;
  SuppThickness     = 4.5*mm;
  AluSeThickness    = 27.2*mm;
  LeadSeThickness   = 13.0*mm;
  PreshowerEnabled = true;
...
//-----------
  ...
  SetZcalAbsMaterial("Tungsten");
  SetZcalSensMaterial("Sci_def");
...
pTung_d =
new G4Material("Tungsten_def", z=74., a=183.84*g/mole, density=19.25*g/cm3);
...
//=======
  ...
  solidWorld = new G4Box("World",                       //its name
                          300.0*cm, 300.0*cm, 1200.0*cm);  //its size
...
//===========================================================
  ...
  double zeroLayerPos = offsetEcal + offsetHcal - ZeroWrapThick/2.;
  //zero layer goes in front of zcal if enabled
  if(NbOfZcalLayers>0 && CablesThickness==0.) zeroLayerPos -= ZcalCalorThickness;
  ...
  phyWrapZero = new G4PVPlacement(0,                       //no rotation
                  G4ThreeVector(0.,0.,zeroLayerPos),  //position
...
//===============================================
  ...
  else if(NewDeadThickness>0.){ //alternative option for dead material associated with shashlik ECAL
     solidCables = new G4Box("Cables",
                             CalorSizeXY/2.,CalorSizeXY/2.,NewDeadThickness/2.);
    
    logicCables = new G4LogicalVolume(solidCables,
                                      pAlum, //default material Al
                                     "Cables");
    
    //directly behind ECAL G10 plate
    middleCables = offsetEcal + EcalCalorThickness + G10plateThickness + NewDeadThickness/2.; 
...
// Zero Calorimeter directly in front of HCAL (optional)
//======================================================
  solidZcal=0; logicZcal=0; physiZcal=0;
  solZcalLayer=0; logZcalLayer=0; phyZcalLayer=0;
  middleZcal = offsetEcal + offsetHcal - ZcalCalorThickness/2.;
  solZcalAbs=0; logZcalAbs=0; phyZcalAbs=0;
  solZcalSens=0; logZcalSens=0; phyZcalSens=0;
  solidZcalWrap=0; logicZcalWrap=0; physiZcalWrap=0;
  solidZcalGap=0; logicZcalGap=0; physiZcalGap=0;
  
  if(NbOfZcalLayers>0 && CablesThickness==0.){ //cannot coexist with (old) dead material
    solidZcal = new G4Box("Zcal",                                  
                    CalorSizeXY/2.,CalorSizeXY/2.,ZcalCalorThickness/2.);
                           
    logicZcal = new G4LogicalVolume(solidZcal,                 
                                    defaultMaterial,           
                                    "Zcal");                   
                                     
    physiZcal = new G4PVPlacement(0,                
                                  G4ThreeVector(0.0, 0.0, middleZcal),        
                                  logicZcal,       
                                  "Zcal",          
                                  logicWorld,       
                                  false,            
    ...
    // Zcal layer
    //------------
    solZcalLayer = new G4Box("ZcLayer",                 
                   CalorSizeXY/2.,CalorSizeXY/2.,ZcalLayerThickness/2.); 
                     
    logZcalLayer = new G4LogicalVolume(solZcalLayer,      
                                       defaultMaterial,
                                       "ZcLayer");        
    if (NbOfZcalLayers > 1)                                      
      phyZcalLayer = new G4PVReplica("ZcLayer",           
                                      logZcalLayer,        
                                      logicZcal,       
                                      kZAxis,            
                                      NbOfZcalLayers,        
      ...
      phyZcalLayer = new G4PVPlacement(0,               
                                       G4ThreeVector(),   
                                       logZcalLayer,                   
                                       "EcLayer",          
                                       logicZcal,       
    ...
    // Zcal absorber material
    //-----------------------
    solZcalAbs = new G4Box("ZcalAbs",            
                            CalorSizeXY/2.,CalorSizeXY/2.,ZcalAbsThickness/2.); 
                        
    logZcalAbs = new G4LogicalVolume(solZcalAbs,    
                                     ZcalAbsMaterial, 
                                     ZcalAbsMaterial->GetName()); 
                                        
    phyZcalAbs = new G4PVPlacement(0,                 
	...
	// Al Wrapper 8.0*mm for scintillator (1.0*mm wall thickness)
	//===========================================================
	solidZcalWrap = new G4Box("Wrapper",
						 CalorSizeXY/2.,CalorSizeXY/2.,WrapThickness/2.);
	
	logicZcalWrap = new G4LogicalVolume(solidZcalWrap,
								  pAlum,
								 "Wrapper");
								   
	physiZcalWrap = new G4PVPlacement(0,                     //no rotation
				  G4ThreeVector(0.,0.,ZcalAbsThickness/2.), //its position
								logicZcalWrap,             //its logical volume$
							   "Wrapper",              //its name
								logZcalLayer,            //its mother
								false,                 //no boulean operat
								0);                    //copy number

	// Air gap 6.0*mm inside Al wrapper
	//=================================								  
	solidZcalGap = new G4Box("Gap",
						CalorSizeXY/2.,CalorSizeXY/2.,GapThickness/2.);
	 
	logicZcalGap = new G4LogicalVolume(solidZcalGap,
								 pAir,
								"Gap");

	physiZcalGap = new G4PVPlacement(0,                       //no rotation
							   G4ThreeVector(0.,0.,0.), //its position
							   logicZcalGap,                //its logical volume
							  "Gap",                    //its name
							   logicZcalWrap,               //its mother
							   false,                   //no boulean operat
							   0);                      //copy number


    // Zcal sensitive material
    //-------------------------
    solZcalSens = new G4Box("ZcalSens",
                      CalorSizeXY/2.,CalorSizeXY/2.,ZcalSensThickness/2.);
                         
    logZcalSens = new G4LogicalVolume(solZcalSens,
                                      ZcalSensMaterial,
                                      ZcalSensMaterial->GetName());
                                   
    phyZcalSens = new G4PVPlacement(0,                     
                      G4ThreeVector(0.,0.,0.),  
                                    logZcalSens,                       
                                    ZcalSensMaterial->GetName(), 
                                    logicZcalGap,
...
//  logicAbsorber->SetVisAttributes(atb);}
 ...
 if(NbOfHcalLayers>0){G4VisAttributes* atb= new G4VisAttributes(G4Colour(1.0,1.0,0.0));
  atb->SetForceSolid(true);
//  logicWrap->SetVisAttributes(atb);}
  logicGap->SetVisAttributes(atb);}
//  logicSens->SetVisAttributes(atb);}
  
 if(NbOfHcalLayers>0){G4VisAttributes* atb= new G4VisAttributes(G4Colour(0.0,1.0,1.0));
  atb->SetForceSolid(true);
  logSensZero->SetVisAttributes(atb);}

 if(CablesThickness>0. || NewDeadThickness>0.){G4VisAttributes* atb= new G4VisAttributes(G4Colour(1.0,0.0,1.0));
  atb->SetForceSolid(true);
 ...
 if(PreshowerEnabled){G4VisAttributes* atb= new G4VisAttributes(G4Colour(0.0,0.0,1.0));
  atb->SetForceSolid(true);
  logicAluSe->SetVisAttributes(atb);}

 if(PreshowerEnabled){G4VisAttributes* atb= new G4VisAttributes(G4Colour(1.0,0.0,0.0));
  atb->SetForceSolid(true);
  logicLeadSe->SetVisAttributes(atb);}
  
  if(NbOfZcalLayers>0 && CablesThickness==0.){G4VisAttributes* atb= new G4VisAttributes(G4Colour(0.9,0.9,0.9));
  atb->SetForceSolid(true);
  logZcalAbs->SetVisAttributes(atb);}

  if(NbOfZcalLayers>0 && CablesThickness==0.){G4VisAttributes* atb= new G4VisAttributes(G4Colour(0.0,1.0,1.0));
  atb->SetForceSolid(true);
  logZcalSens->SetVisAttributes(atb);}
  
  if(EcalAbsThickness > 0.){G4VisAttributes* atb= new G4VisAttributes(G4Colour(0.9,0.9,0.9));
  atb->SetForceSolid(true);
  logEcalAbs->SetVisAttributes(atb);}
  ...
  if(NbOfHcalLayers>0){G4VisAttributes* atb= new G4VisAttributes(G4Colour(0.86,0.79,0.32));
  atb->SetForceSolid(true);
  logicAbsorber->SetVisAttributes(atb);}
...
void DetectorConstruction::PrintCalorParameters()
{
  ...
  G4cout << "\n------------------------------------------------------------"
         ...
         << " ] " 
		 << "\n---> [ "
		 << HcalAbsMaterial->GetName() << " interaction length: " << HcalAbsMaterial->GetNuclearInterLength()
		 << " ] "
         << "\n------------------------------------------------------------\n";
  }
  G4cout << "\n------------------------------------------------------------"
...
void DetectorConstruction::SetHcalAbsMaterial(G4String materialChoice)
{
  ...
  if(HcalAbsMaterial->GetName() != "Brass_def" && HcalAbsMaterial->GetName() != "Brass" && HcalAbsMaterial->GetName() != "Tungsten" && HcalAbsMaterial->GetName() != "Tungsten_def")  
    { G4cout << "\n ===> Stop from DetectorConstruction::SetHcalAbsMaterial(): "
              << "\n HcalAbsMaterial = " << HcalAbsMaterial->GetName()  
              << "\n Brass or Tungsten only possible for the Hcal "
...
void DetectorConstruction::SetEcalAbsThickness(G4double val)
{
  ...
  if (NbOfHcalLayers>0 && EcalAbsThickness > 400.000)
  ...
  if (NbOfHcalLayers>0 && EcalAbsThickness > 220.001)
...
void DetectorConstruction::SetEcalSensThickness(G4double val)
{
  ...
  if (NbOfHcalLayers>0 && EcalSensThickness > 400.000)
  ...
  if (NbOfHcalLayers>0 && EcalSensThickness > 220.000)
...
// Set HCAL offset from ECAL
void DetectorConstruction::SetHcalOffset(G4double val)
{
  offsetHcal = val;
  if (offsetHcal < 400.000)
    { G4cout << "\n ===> Stop from DetectorConstruction::SetHcalOffset(): " 
             << "\n offsetHcal = " << offsetHcal  
             << " mm, less than 400.0 mm. "
             << "\n Check value of offsetHcal." 
             << G4endl;
      exit(1);
    }
}

// change Number of Hcal layers and recompute the calorimeter parameters
//-----------------------------------------------------------------------
void DetectorConstruction::SetNbOfHcalLayers(G4int val)
{
  NbOfHcalLayers = val;
  if (NbOfHcalLayers < 0)
    { G4cout << "\n ===> Stop from DetectorConstruction::SetNbOfHcalLayers(): " 
             << "\n NbOfHcalLayers = " << NbOfHcalLayers
             << " less than 0. "
             << "\n Check value of NbOfHcalLayers. " 
             << G4endl;
      exit(1);
    }
}

// change Hcal absorber thickness
//-------------------------------------------------------------------------
void DetectorConstruction::SetHcalAbsThickness(G4double val)
{
  AbsorberThickness = val;
  if (AbsorberThickness > 400.000)
    { G4cout << "\n ===> Stop from DetectorConstruction::SetHcalAbsThickness(): " 
             << "\n AbsorberThickness = " << AbsorberThickness  
             << " mm, greater than 400.0 mm. "
             << "\n Check value of AbsorberThickness." 
             << G4endl;
      exit(1);
    }
  if (AbsorberThickness > 300.000)
    { G4cout << "\n ===> Warning from DetectorConstruction::SetHcalAbsThickness(): "
             << "\n AbsorberThickness = " << AbsorberThickness
             << " mm, greater than 300.0 mm. "
             << "\n Are you shure that it is correct ? "
             << G4endl;
    }
}
...
// search the material by its name
//---------------------------------
void DetectorConstruction::SetZcalAbsMaterial(G4String materialChoice)
{
  G4Material* pttoMaterial = G4Material::GetMaterial(materialChoice);     
  if (pttoMaterial) ZcalAbsMaterial = pttoMaterial;
}

// change Zcal absorber thickness and recompute the calorimeter parameters
//-------------------------------------------------------------------------
void DetectorConstruction::SetZcalAbsThickness(G4double val)
{
  ZcalAbsThickness = val;
  if (ZcalAbsThickness > 400.000)
    { G4cout << "\n ===> Stop from DetectorConstruction::SetZcalAbsThickness(): " 
             << "\n ZcalAbsThickness = " << ZcalAbsThickness  
             << " mm, greater than 400.0 mm. "
             << "\n Check value of ZcalAbsThickness." 
             << G4endl;
      exit(1);
    }
  if (ZcalAbsThickness > 300.000)
    { G4cout << "\n ===> Warning from DetectorConstruction::SetZcalAbsThickness(): "
             << "\n ZcalAbsThickness = " << ZcalAbsThickness
             << " mm, greater than 300.0 mm. "
             << "\n Are you shure that it is correct ? "
             << G4endl;
...
// search the material by its name
//--------------------------------
void DetectorConstruction::SetZcalSensMaterial(G4String materialChoice)
{
  G4Material* pttoMaterial = G4Material::GetMaterial(materialChoice);
  if (pttoMaterial) ZcalSensMaterial = pttoMaterial;

  if( ZcalSensMaterial->GetName() != "PbWO_def" && ZcalSensMaterial->GetName() != "LAG"
    && ZcalSensMaterial->GetName() != "YAG"  && ZcalSensMaterial->GetName() != "LSO"
    && ZcalSensMaterial->GetName() != "YSO"  && ZcalSensMaterial->GetName() != "LYSO"
    && ZcalSensMaterial->GetName() != "PbWO" && ZcalSensMaterial->GetName() != "LYSOc"
	&& ZcalSensMaterial->GetName() != "Sci_def" && ZcalSensMaterial->GetName() != "Scintillator")
     { G4cout << "\n ===> Stop from DetectorConstruction::SetZcalSensMaterial(): "
              << "\n ZcalSensMaterial = " <<  ZcalSensMaterial->GetName()
              << "\n Please choose Zcal sensitive material from "
              << "this possible list:"
              << "\n PbWO, LAG, YAG, LSO, YSO, LYSO and LYSOc"
              << G4endl;
       exit(1);
...
// change Zcal sensitive thickness and recompute the calorimeter parameters 
//--------------------------------------------------------------------------
void DetectorConstruction::SetZcalSensThickness(G4double val)
{
  ZcalSensThickness = val;
  if (ZcalSensThickness > 400.000)
    { G4cout << "\n ===> Stop from DetectorConstruction::SetZcalSensThickness(): " 
             << "\n ZcalSensThickness = " << ZcalSensThickness  
             << " mm, greater than 400.0 mm "
             << "\n Check value of ZcalSensThickness. "  
             << G4endl;
      exit(1);
    }
  if (ZcalSensThickness > 300.000)
    { G4cout << "\n ===> Warning from DetectorConstruction::SetZcalSensThickness(): "
             << "\n ZcalSensThickness = " << ZcalSensThickness
             << " mm, greater than 300.0 mm "
             << "\n Are you shure that it is correct ? "
             << G4endl;
...
// change Number of Zcal layers and recompute the calorimeter parameters
//-----------------------------------------------------------------------
void DetectorConstruction::SetNbOfZcalLayers(G4int val)
{
  NbOfZcalLayers = val;
  if (NbOfZcalLayers < 0)
    { G4cout << "\n ===> Stop from DetectorConstruction::SetNbOfZcalLayers(): " 
             << "\n NbOfZcalLayers = " << NbOfZcalLayers  
             << " less than 0. "
             << "\n Check value of NbOfZcalLayers. " 
             << G4endl;
      exit(1);
    }
}
...
// change new dead material thickness
//-------------------------------
void DetectorConstruction::SetNewDeadThickness(G4double val)
{
  NewDeadThickness = val;
  if (NewDeadThickness > 200.0)
    { G4cout << "\n ===> Stop from DetectorConstruction::SetNewDeadThickness(): " 
             << "\n NewDeadThickness = " << NewDeadThickness  
             << " mm, greater than 200.0 mm "
             << "\n Check value of NewDeadThickness. "  
             << G4endl;
      exit(1);
    }
}
...
// disable preshower
//------------------
void DetectorConstruction::SetPreshower(G4bool val){
...
void DetectorConstruction::SetHcalZeroThickness(G4double val){
	ZeroSensThick = val;
	if(ZeroSensThick > ZeroGapThick)
	{ G4cout << "\n ===> Stop from DetectorConstruction::SetHcalZeroThickness(): " 
             << "\n ZeroSensThick = " << ZeroSensThick  
             << " mm, greater than ZeroGapThick = " << ZeroGapThick << " mm "
             << "\n Check value of ZeroSensThick. "  
             << G4endl;
      exit(1);
    }
}
